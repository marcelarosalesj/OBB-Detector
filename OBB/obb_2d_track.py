...
def draw_bounding_polygon_with_label(frame, box, label, track_id, color):
    ...
    cv2.polylines(
        frame,
        [np.array(box, np.int32).reshape((-1, 1, 2))],
        isClosed=True,
        color=color,
        thickness=2,
    )
...
# Load a model
model_2d = YOLO("/home/marcela/repos/OBB-Detector/OBB/yolov9e.pt")  # load a built-in model
model_obb = YOLO("/home/marcela/repos/OBB-Detector/OBB/runs/obb/train8/weights/best.pt")  # Your custom model
# Predict with the model
video = cv2.VideoCapture("/home/marcela/repos/OBB-Detector/OBB/test1.mp4")
...
colors = {
    ...
    for class_id in labels_map_coco.keys()
}
...
for i in tqdm(range(frames), desc="Processing frames"):
    ...
    if not ret:
        continue
    results_obb = model_obb.track(
        frame, persist=True, conf=0.3, iou=0.5, tracker="botsort.yaml", save=False
    )  # predict on an image
    results_2d = model_2d.track(
        frame, persist=True, conf=0.3, iou=0.5, tracker="botsort.yaml", save=False
    )  # predict on an image
    # Get result values
